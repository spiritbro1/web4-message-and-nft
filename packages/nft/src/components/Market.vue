...
this.isLoading=true;
      ...
      const imageName=`${uuidv4()}.${this.imgName.split(".")[this.imgName.split(".").length-1]}`;
       const joko=await fetch(`${process.env.VUE_APP_API_URL}/api/copin`, {
    ...
    if(response.ok){
      const res = await fetch(`${process.env.VUE_APP_API_URL}/api/head`, { headers:{'Content-Type': "application/json"},method: "POST", body:JSON.stringify({key:imageName})});
      const data=await res.json();
      
      await this.contract.nft_mint({
         token_id: `${jumlah+1}`,
    metadata: {title:this.title,description:this.desc,media:`https://dweb.link/ipfs/${data.Metadata.cid}`},
